...
var portfolio_row_attrs = ['categories', 'column', 'count', 'bgcolor', 'ratio', 'filter', 'order', 'exclude', 'extra_class', 'visibility'];

function get_blox_element_portfolio_row($content, $attrs){
    
    return '<div class="blox_item portfolio_row type-fullwidth" '+($attrs!=undefined ? $attrs : '')+'> \
            <div class="blox_item_title"> \
                <i class="fa-th"></i> \
                <span class="blox_item_title">Portfolio Fullwidth</span> \
                '+ get_blox_actions() +' \
            </div> \
...
function parse_shortcode_portfolio_row($content){
    $content = wp.shortcode.replace( 'portfolio_row', $content, function(data){
        var attrs = '';
        ...
        return get_blox_element_portfolio_row( data.content, attrs );
    });
    return $content;
}


function revert_shortcode_portfolio_row($content){
    $content.find('.portfolio_row').each(function(){
        attr = '';
        var temp_val = '';

        for (var i = 0; i < portfolio_row_attrs.length; i++) {
            temp_val = jQuery(this).attr(portfolio_row_attrs[i])+'';
            if( temp_val!='undefined' && temp_val!='' ){
                attr += ' '+ portfolio_row_attrs[i] +'="'+ temp_val +'"';
            }
        }
		
        jQuery(this).replaceWith('[portfolio_row'+attr+'/]');
    });
    return $content;
}


function add_event_blox_element_portfolio_row(){
	
    jQuery('.portfolio_row').each(function(){
        ...
        .click(function(){
            ...
            add_event_blox_element_portfolio_row();
        });
